...
class Prep_and_Modeling():
    def __init__(self, audio_file, model_file):
        ...
        self.load_model = self.model.load_state_dict(torch.load(model_file, map_location=torch.device('cpu')))
        self.model.eval()

    def preprocess(self):
        input = self.processor(self.audio, sampling_rate=16000, return_tensor='pt')
